...
void Image::image_filter_parallel_mean(int *rbuf,int width,int height,int windowSize)
{
   // make copy of the existing image -- with borders
...
void Image::image_filter_median_parallel(int *rbuf,int height,int width,int windowSize)
{
   ...
   for (int x = 0; x < width; x++) {
      for (int y = 0; y < height; y++) {
         ...
         sort (colorArray.begin(), colorArray.end());
         assert(colorArray.size() != 0);
         rbuf[(width*y)+x] = colorArray[colorArray.size()/2];
      }
   }
}